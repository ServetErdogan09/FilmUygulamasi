...
class KomediFilmAdapter(val mContext: Context, val animasyonListe: List<Movie>) : RecyclerView.Adapter<KomediFilmAdapter.AnimasyonFilmTutucu>() {

    class AnimasyonFilmTutucu(val tasarim : KategoriDuzenRecyclerBinding) : RecyclerView.ViewHolder(tasarim.root){

    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AnimasyonFilmTutucu {
        val tasarim = KategoriDuzenRecyclerBinding.inflate(LayoutInflater.from(parent.context),parent,false)
        return AnimasyonFilmTutucu(tasarim)
    }

    override fun onBindViewHolder(holder: AnimasyonFilmTutucu, position: Int) {
        val kategoriFilm = animasyonListe[position]
