...
class GerilimFilmAdapter(val mContext: Context, val aileListe : List<Movie>) :
    RecyclerView.Adapter<GerilimFilmAdapter.AileFilmTutucu>() {

    class AileFilmTutucu(val tasarim : KategoriDuzenRecyclerBinding) : RecyclerView.ViewHolder(tasarim.root){

    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AileFilmTutucu {
        val tasarim = KategoriDuzenRecyclerBinding.inflate(LayoutInflater.from(parent.context),parent,false)
        return AileFilmTutucu(tasarim)
    }

    override fun getItemCount(): Int {
         return aileListe.size
    }

    override fun onBindViewHolder(holder: AileFilmTutucu, position: Int) {
        val kategoriFilm = aileListe[position]
